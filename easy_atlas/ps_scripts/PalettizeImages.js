...
for (i = 0; i < argList.length; i += 5) {
  ...
  for (k = 0; k < docTemp.channels.length; k += 1) {
    if (docTemp.channels[k].kind != ChannelType.MASKEDAREA) {
      continue;
    }
  ...
  for (k = docTemp.layers.length - 1; k >= 0; k -= 1) {
    ...
    if (docTemp.activeLayer.typename == "LayerSet") {
      ...
      docTemp.activeLayer.duplicate(
        newLayerSet.layers[0],
    ...
    if (k == 0 && alpha == true) {
      ...
      selectedRegion = Array(
        Array(LB[0].value, LB[1].value),
        Array(LB[2].value, LB[1].value),
        Array(LB[2].value, LB[3].value),
        Array(LB[0].value, LB[3].value)
      );
      outputDoc.selection.select(selectedRegion);
      outputDoc.paste(true);
      outputDoc.activeChannels = [
        outputDoc.channels[0],
        outputDoc.channels[1],
        outputDoc.channels[2],
      ];
...
switch (fileExtension.toLowerCase()) {
  ...
  default:
    saveOptions = null;
    alert(
      "File extension not supported by Easy Atlas. This document will not be saved."
    );
}
